...
if __name__ == "__main__":
	...
	rp_mushroom_new_data = pd.DataFrame(np.hstack((X2,np.atleast_2d(y).T)))
	cols = list(range(rp_mushroom_new_data.shape[1]))
	cols[-1] = 'class'
	rp_mushroom_new_data.columns = cols
	rp_mushroom_new_data.to_csv('rp_mushroom_new_data.csv')
	###KMeans###
	...
	ax.set_title('Mushroom: K-Means Clustering (after RP)')
	ax.set_xlabel('RP1')
	ax.set_ylabel('RP2')
	...
	ax2.set_title('Mushroom: EM Clustering (after RP)')
	ax2.set_xlabel('RP1')
	ax2.set_ylabel('RP2')
