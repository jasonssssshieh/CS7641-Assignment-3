...
if __name__ == "__main__":
    ...
    ax.set_title('Abalone: ICA Kurtosis')
    plt.show()
	...
	ica_abalone_new_data = pd.DataFrame(np.hstack((X2,np.atleast_2d(y).T)))
	cols = list(range(ica_abalone_new_data.shape[1]))
	cols[-1] = 'rings'
	ica_abalone_new_data.columns = cols
	ica_abalone_new_data.to_csv('ica_abalone_new_data.csv')
...
###EM####
    ...
    ax.set_title('Abalone: K-Means Clustering (after ICA)')
    ax.set_xlabel('IC1')
    ax.set_ylabel('IC2')
	...
	scatter2 = ax2.scatter(X2[:,5], X2[:,6], c = labels[0], s = 10, cmap='viridis')
	#plt.scatter(cent_em[:, 0], cent_em[:, 1], c='black', s=200, alpha=0.5)
	ax2.set_title('Abalone: EM Clustering (after ICA)')
	ax2.set_xlabel('IC1')
	ax2.set_ylabel('IC2')
